# JFEM Results Comparison Tool
# Usage: julia compare_results.jl <model_name> [output_dir]
# Compares <model_name>.NAST.json vs <model_name>.JU.JSON in the output directory
...
function analyze_chunk_pair(ref_chunk, test_chunk, fields, label)
    if isempty(ref_chunk) || isempty(test_chunk); return; end
...
function compare_category(ref_data, test_data, top_key, sub_key, id_key, fields, label)
    ...
    for i in 1:n_compare
        ...
        r_c = r_chunks[i]
        t_c = t_chunks[i]
        if top_key in ["stresses", "strains"]
            ...
            analyze_chunk_pair(flatten(r_c), flatten(t_c), fields, label)
        else
...
function run_full_comparison(model_name::String, output_dir::String)
    ref_file = joinpath(output_dir, "$(model_name).NAST.json")
    ...
    compare_category(ref, test, "forces", "cbar", "eid",
        ["axial", "shear_1", "shear_2", "torque", "moment_a1", "moment_b1"], "CBAR FORCES")
...
if length(ARGS) >= 1
    model_name = ARGS[1]
    output_dir = length(ARGS) >= 2 ? ARGS[2] : joinpath(@__DIR__, "..", "FEM_output")
    run_full_comparison(model_name, output_dir)
else
    println("Usage: julia compare_results.jl <model_name> [output_dir]")
    println("  Compares <model_name>.NAST.json vs <model_name>.JU.JSON")
    println("  output_dir defaults to FEM_output/ next to the src/ folder")
end
